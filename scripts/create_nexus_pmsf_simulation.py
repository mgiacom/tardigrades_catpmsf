...
my_tree_length = float(sys.argv[3])  # tree length according to the .iqtree output
...
# Iterate through the dictionary and construct the Poisson+SSFF+G+tree_length string for each site
for idx, (k, v) in enumerate(my_dict.items()):
    freq = '/'.join(v[0])        # Frequency values (list of strings)
    tree_len = str(v[1] / my_tree_length)  # Scaled tree length
